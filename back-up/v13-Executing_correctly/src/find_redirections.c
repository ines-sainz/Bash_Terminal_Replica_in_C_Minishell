...
void	fill_here_doc(int fd, t_execution *iter_exe, t_mini *mini)
{
	if (iter_exe->inf_pipe < 0 || iter_exe->outf_pipe < 0)
	{
		printf("3 here_doc close: %i\n", fd);//
		close(fd);
		return ;
	}
	if (iter_exe->inf_pipe != 0)
	{
		printf("4 inf close: %i\n", iter_exe->inf_pipe);//
		close(iter_exe->inf_pipe);
	}
	iter_exe->inf_pipe = fd;
	printf("5 here_doc set: %i\n", fd);//
	if (iter_exe->inf_pipe < 0)
	{
...
void	fill_pipe(t_execution *iter_exe)
{
	...
	pipe(pipe_fds);
	printf("10 inf %i, outf %i\n", pipe_fds[0], pipe_fds[1]);
	if (iter_exe->outf_pipe == 1)
	{
		printf("11 pipe set outf: %i\n", pipe_fds[1]);
		iter_exe->outf_pipe = pipe_fds[1];
	}
	else if (iter_exe->outf_pipe != 1 && iter_exe->outf_pipe > 0)
	{
		printf("12 pipe outf close: %i\n", pipe_fds[1]);
		close(pipe_fds[1]);
		printf("13 out_still: %i\n", iter_exe->outf_pipe);
	}
	iter_exe = iter_exe->next;
	if (iter_exe->inf_pipe == 0)
	{
		iter_exe->inf_pipe = pipe_fds[0];
		printf("14 pipe set inf: %i\n", iter_exe->inf_pipe);
	}
	else if (iter_exe->inf_pipe > 0)
	{
		printf("15 inf close: %i\n", pipe_fds[0]);
		close(pipe_fds[0]);
		printf("16 inf_still: %i\n", iter_exe->inf_pipe);
	}
}
